...
// Comprehensive list of Indian cities with coordinates (same as in generateSampleData.ts)
const cities = [
  // Major Metro Cities
...
function generatePincode(state: string): string {
  const statePincodes: { [key: string]: string[] } = {
    ...
    Haryana: ["122", "124", "125"],
    Punjab: ["140", "141", "142"],
    "Jammu and Kashmir": ["190", "191", "192"],
    Jharkhand: ["814", "815", "816"],
    Chandigarh: ["160"],
    Assam: ["781", "782", "783"],
    Uttarakhand: ["248", "249", "263"],
    Odisha: ["751", "752", "753"],
    Kerala: ["682", "695", "673"],
    Chhattisgarh: ["492", "493", "494"],
  };
...
async function generateUsers(count: number = 200) {
...
async function generateRoomsInBatches(
  users: unknown[],
  totalRooms: number = 15000
) {
  console.log(`Generating ${totalRooms} rooms in batches...`);
  ...
  const totalBatches = Math.ceil(totalRooms / batchSize);
  let totalCreated = 0;

  for (let batch = 0; batch < totalBatches; batch++) {
    const roomsInThisBatch = Math.min(batchSize, totalRooms - totalCreated);
    const rooms = [];

    console.log(
      `Processing batch ${batch + 1
      }/${totalBatches} (${roomsInThisBatch} rooms)...`
    );

    for (let i = 0; i < roomsInThisBatch; i++) {
      ...
      rooms.push({
        ...
        description: `Beautiful ${roomType} room available for rent in ${city.name
            ...
            : roomType === "shared"
              ...
              : "comfortable living"
          }. Located in a prime area with easy access to public transport, markets, and restaurants. The property offers excellent connectivity and is situated in a safe neighborhood.`,
        monthlyRent: rent,
        location: {
          address,
          coordinates: {
    ...
    console.log(
      `Batch ${batch + 1} completed: ${createdRooms.length
      } rooms created (Total: ${totalCreated})`
    );
  }

  console.log(`All batches completed! Total rooms created: ${totalCreated}`);
  return totalCreated;
}
...
export async function generateLargeDataset() {
  try {
    ...
    console.log("This will create 15,000+ rooms across 70+ cities");
    ...
    // Generate rooms in batches
    const totalRooms = await generateRoomsInBatches(users, 15000);
    ...
    return { users, totalRooms, cities: cities.length };
  } catch (error) {
...
if (require.main === module) {
  generateLargeDataset()
    .then((result) => {
      ...
      console.log(
        `Summary: ${result.users.length} users, ${result.totalRooms} rooms across ${result.cities} cities`
      );
      process.exit(0);
    })
    .catch((error) => {
